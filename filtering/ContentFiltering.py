...
def evaluate(user, LIKES, similar_users):
    ...
    print shows_recommended, "shows_recommended"
...
def main(argv):
    # outFile = open('user_data_nb.txt', 'w')
    if len(argv) > 1:
    ...
    tv_genre = []
    tv_text = []
    with open('../data/tv_shows.txt', 'r') as f:
        lines = f.readlines()
        for line in lines:
            l = line.strip().split('\t')
    ...
    for uid in LIKES.keys():
        ...
        for showId in LIKES[uid]:
            ...
            for idx, title in enumerate(tv_titles):
                # print "enumerate!!" idx
                # print "title",tv_titles
                if len(set(show_title).intersection(title)) > 0:
                    # print "id!!", idx
                    showNumList.append(idx)
        ...
        # if len(showNumList) > 0:
        #     listTpMatrix = np.zeros(347)
        #     for i in showNumList:
        #         listTpMatrix[i] = 1
        #     # showMatrix.append(showNumList)
        #     strings = ["%.0f" % number for number in listTpMatrix]
        #     print strings
        #     print ','.join(strings)
        #     showMatrix.append(listTpMatrix)

        #     # np.savetxt(outFile, ','.join(strings), newline=" ")
        #     outFile.write("%s\n" % ','.join(strings))
